...
const MapScreen = ({route}) => {
  ...
  const routeCoordinates = data ? decodePolyline(data.routes[0].overview_polyline.points) : [];
    ...
    return (
      <View style={{ flex: 1 }}>
        ...
        {error && <Text style={{ textAlign: 'center', color: 'red', marginTop: 10 }}>{error.message}</Text>}
        {data && <Text style={{ textAlign: 'center', marginTop: 10 }}>{`Estimated time: ${data.routes[0].legs[0].duration.text}`}</Text>}
      </View>
    );
  };
